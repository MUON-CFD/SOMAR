...
{
  for (unsigned int i = 0; i < a_oldItems.size(); ++i)
    {
  ...
  if (isPeriodic)
    {
      ...
      if (multipleWraps)
        {
          shiftIt.computeShifts(a_domain.isPeriodicVect(), numWraps);
        }
                            ...
                            const IntVect& a_ghost)
{
  ...
  clear();
  m_isDefined = true; // Added by ES 5/19/2021
...
{
  std::sort(
      m_localMotionPlan.begin(), m_localMotionPlan.end(), MotionItemSorter());
  std::sort(
      m_fromMotionPlan.begin(), m_fromMotionPlan.end(), MotionItemSorter());
  std::sort(
    m_toMotionPlan.begin(), m_toMotionPlan.end(), MotionItemSorter());
}
