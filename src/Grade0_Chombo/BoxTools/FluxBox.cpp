...
{
  void* __attribute__((unused)) buffer;
  buffer = linearOut2(buf, R, comps);
}
...
#ifdef CH_USE_PYTHON
PyObject *FluxBox::pack() const {
  PyObject *pFBArgs = PyTuple_New(4 + SpaceDim+1);
  {
    ...
    for (auto i = 0; i < SpaceDim; ++i) {
      ...
      PyTuple_SetItem(pSize, i, PyLong_FromLong(this->box().size()[i]));
      PyTuple_SetItem(pCentering, i, PyLong_FromLong(ind.test(i)));
    }
    PyTuple_SetItem(nComp, 0, PyLong_FromLong(1));

    PyTuple_SetItem(pFBArgs, 0, pLo);
    PyTuple_SetItem(pFBArgs, 1, pSize);
    PyTuple_SetItem(pFBArgs, 2, pCentering);
    PyTuple_SetItem(pFBArgs, 3, nComp);
  }

  for (int dim = 0; dim < SpaceDim; ++dim) {
    const FArrayBox &a_in = this->operator[](dim);
    PyTuple_SetItem(pFBArgs, 4 + dim, a_in.pack());
  }
  std::string Label = "FluxBox";
  PyObject *pLabel = PyUnicode_FromString(Label.c_str());
  PyTuple_SetItem(pFBArgs, 4+ SpaceDim, pLabel);
  return pFBArgs;
}

PyObject *FluxBox::pack() {
  PyObject *pFBArgs = PyTuple_New(4 + SpaceDim+1);
  {
    PyObject *pLo = PyTuple_New(SpaceDim);
    PyObject *pSize = PyTuple_New(SpaceDim);
  ...
  for (int dim = 0; dim < SpaceDim; ++dim) {
    FArrayBox &a_in = this->operator[](dim);
    PyTuple_SetItem(pFBArgs, 4 + dim, a_in.pack());
  }
  std::string Label = "FluxBox";
  PyObject *pLabel = PyUnicode_FromString(Label.c_str());
