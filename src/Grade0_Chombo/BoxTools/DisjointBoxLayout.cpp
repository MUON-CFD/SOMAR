...
DisjointBoxLayout::computeNeighbors()
{
        ...
        for (lit.reset(); lit.ok(); ++lit, ++index) {
            ...
            if (!periodicTestBox.contains(box)) {
                ...
                for (shiftIt.begin(); shiftIt.ok(); ++shiftIt) {
                    ...
                    periodicImages.push_back(
                        std::pair<int, LayoutIndex>(shiftIt.index(), lit()));
                    shiftedBox.shift(-shiftVect);
                }  // end loop over periodic shift directions
            }      // end if periodic
        }
    }
    //  OK, for this work, we don't use a standard LayoutIterator, since I want
    //  to
    // be pushing the start of the window forward, which LayoutIterator is not
    // built to do.
    unsigned int id    = 0;
    ...
    for (DataIterator dit = dataIterator(); dit.ok(); ++dit) {
        const Box gbox = Box(get(dit())).grow(1);

        auto& neighbors = (*m_neighbors)[dit().intCode()];
        ...
        for (id = start; id < end; ++id) {
            if (id != index(dit())) {
                ...
                if (gbox.intersectsNotEmpty(b)) {
                    neighbors.push_back(
                        std::pair<int, LayoutIndex>(-1, vecLayoutIndex[id]));
                }
            }
        }
        // now run through periodic boxes.
        for (auto it = periodicImages.begin(); it != periodicImages.end(); ++it) {
...
#ifdef CH_USE_PYTHON
DisjointBoxLayout::DisjointBoxLayout(PyObject *a_pin)
:DisjointBoxLayout()
{
  ...
  for (auto i = 1; i<n+1; ++i)
  {
    ...
    PIDs[i-1]=static_cast<int>(PyLong_AsLong(PyTuple_GetItem(a_pin,2*i)));
  }
  ProblemDomain Domain(PyTuple_GetItem(a_pin, 2*n+1));
  define(boxes,PIDs, Domain);

}



PyObject *DisjointBoxLayout::pack() const
{

  PyObject *pArg = PyTuple_New(3+1);
  PyObject *pBoxes=PyTuple_New(this->size());
  PyObject *pIds=PyTuple_New(this->size());
  PyObject *pDomain=PyTuple_New(1);
  int i=0;
  for (auto LI=this->layoutIterator(); LI.ok(); ++LI, ++i)
  {
    ...
    ProblemDomain Domain = this->physDomain();
    PyTuple_SetItem(pDomain,0, Domain.pack());

    PyTuple_SetItem(pArg, 0, pBoxes);
    PyTuple_SetItem(pArg, 1, pIds);
    PyTuple_SetItem(pArg, 2, pDomain);
    std::string Label = "DisjointBoxLayout";
    PyObject *pLabel = PyUnicode_FromString(Label.c_str());
    PyTuple_SetItem(pArg, 3, pLabel);
