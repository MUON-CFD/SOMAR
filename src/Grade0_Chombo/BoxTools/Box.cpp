...
PyObject *Box::pack() const {
  IntVect b_le = this->smallEnd();
  IntVect b_be = this->bigEnd();

  PyObject *pBox = PyTuple_New(2 * SpaceDim+1);
  for (auto i = 0; i < SpaceDim; ++i) {
  ...
  PyTuple_SetItem(pBox, 2 * SpaceDim, pLabel);
  return pBox;
}
#endif
//
