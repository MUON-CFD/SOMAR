...
mergeBoxes(Vector<Vector<Box> >& a_vvbox,
           const int&   a_dir)
{
  for (unsigned int ilev = 0; ilev < a_vvbox.size(); ilev++)
    {
           ...
           const int&   a_dir)
{
          ...
          for (int dir = 0; dir < SpaceDim; dir++)
            {
              if (dir != a_dir)
                {
                  Vector<Box> vboxtemp;
                  for (unsigned int ibox = 0; ibox < a_vboxin.size(); ibox++)
                    {
                      Box& boxThis = a_vboxin[ibox];
                      Box growThis = boxThis;
                      growThis.grow(1);
                      growThis.grow(a_dir,-1);
                      for (unsigned int jbox = 0; jbox < a_vboxin.size(); jbox++)
                        {
                          if (ibox!=jbox)
                            {
  ...
  std::sort(a_vboxin.begin(), a_vboxin.end());
}
