                ...
                const int a_LBnumProc)
{
  ...
  for (unsigned int i = 0; i < a_boxes.size(); ++i)
    {
                ...
                const int                a_LBnumProc)
{
  ...
  for (unsigned int i=0; i<a_computeLoads.size(); i++)
    {
            ...
            )
{
  ...
  if ( Grids.size() != RefRatios.size() )
    { return -1013; }
  for ( unsigned int lvl=0; lvl<Grids.size(); ++lvl )
    {
  ...
  if ( nProc == 1 )
    {
      for ( unsigned int lvl=0; lvl<Grids.size(); ++lvl )
        {
          for ( unsigned int i=0; i<Grids[lvl].size(); ++i )
            {
  ...
  else
    {
      ...
      for ( unsigned int lvl=0; lvl<Grids.size(); ++lvl )
        {
          ...
          for ( unsigned int i=loads.size()-1; i>0; --i )
            {
	  ...
	  if (loads.size()>0) { // i is unsigned!
	    proc_loads[iproc_minload].push_back(loads[0]);
	    total_loads[iproc_minload] += loads[0].load;
	    iproc_minload = min_element(total_loads);
	  }
          // compute average load per processor, truncated to int
          long avg_load = 0;
          for ( unsigned int i=0; i<total_loads.size(); ++i ) avg_load += total_loads[i];
          avg_load /= nProc;
          ...
          int ibmax,jbmax;
	  unsigned int ipmax,jpmax;  //box and processor indices corresponding to max_change
          ...
          while ( 1 )
            {
              ...
              for ( unsigned int j=0; j<proc_loads.size(); ++j )
                {
                  if ( j != ipmax )
                    {
                      ...
                      for ( unsigned int ibox=0; ibox<proc_loads[ipmax].size(); ++ibox )
                        {
                          for ( unsigned int jbox=0; jbox<proc_loads[j].size(); ++jbox )
                            {
                              iter_count++;
          ...
          for ( unsigned int ip=0; ip<proc_loads.size(); ++ip )
            {
              for ( unsigned int jb=0; jb<proc_loads[ip].size(); ++jb )
                {
            ...
            const int    /*a_numProc*/)
{
    ...
    status = MPI_Allgather(nonConstBox,
                           boxSize,
                           MPI_BYTE,
                           &(tempBoxes[0]),
                           boxSize,
    ...
    for (unsigned int i = 0; i < tempBoxes.size(); i++) {
        tempProcAssign[i] = i;
    }
