...
BRMeshRefine::findSplit( const Vector<int>& a_trace, const int a_maxBoxSize ) const
{
  ...
  if (a_trace.size() > 2*(unsigned int)a_maxBoxSize)
  {
    // look for a place to split at -- begin in the middle, count down to 1,
    // look in both directions
    unsigned int ibegin = a_trace.size()/2;
    unsigned int iforward = ibegin+1;
    for ( unsigned int i = ibegin ; (i >= 1 && iforward < a_trace.size()-1 ); i-- )
    {
      if ( a_trace[i] == 0 ) return( i );
  ...
  if (a_trace.size() > 2*(unsigned int)a_maxBoxSize)
    {
      ...
      unsigned int ibegin = a_trace.size()/2;
      unsigned int iforward = ibegin+1;
      for ( unsigned int i = ibegin ; (i >= 2 && iforward < a_trace.size()-1); i-- )
        {
          absval = abs( d2Trace[i-1] - d2Trace[i] ) ;
          if ( d2Trace[i-1] * d2Trace[i] < 0  && absval > a_maxVal )
          {
...
void
BRMeshRefine::sendBoxesParallel(const std::list<Box>& a_mesh, int /*tag*/) const
{
#ifdef CH_MPI
                                   ...
                                   int             tag) const
{
    ...
    if (a_from.size() == a_to.size() || !hang) {
        ...
        // pout()<<"sending "<<ch_count.back()/boxSize<<" boxes to
        // "<<dest<<std::endl;
        MPI_Sendrecv(sendBuffers.back(),
                     ch_count.back(),
                     MPI_INT,
                     dest,
                     tag,
