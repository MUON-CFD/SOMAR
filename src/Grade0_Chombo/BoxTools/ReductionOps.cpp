                 ...
                 const Interval& comps) const
{
  ...
  Box flattenedBox(R);
  for (unsigned int n=0; n<m_summingDir.size(); n++)
    {
                      ...
                      const Interval& comps) const
{
  Real* buffer = (Real*)buf;
  Box flattenedBox(R);
  for (unsigned int n=0; n<m_spreadingDir.size(); n++)
    {
  ...
  if (m_spreadingDir.size() == 1)
    {
      ...
      int __attribute__((unused)) fromBoxHi = RegionFrom.bigEnd(m_spreadingDir[0]);
      ...
      for (int dir=0; dir<SpaceDim; dir++)
        {
          ...
          int __attribute__((unused)) fromBoxHi = RegionFromDir.bigEnd(m_spreadingDir[0]);
