...
NeighborIterator::NeighborIterator(const DisjointBoxLayout& dbl)
  :m_dblPtr(&dbl), m_lindex(-1, -1, dbl.m_layout.get())
{
}

void
NeighborIterator::begin(const DataIndex& a_dataIndex)
{
    const Vector<Vector<std::pair<int, LayoutIndex>>>& vv =
        *(m_dblPtr->m_neighbors);
    const std::vector<std::pair<int, LayoutIndex>>& v =
        vv[a_dataIndex.intCode()];
