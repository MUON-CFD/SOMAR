                   ...
                   const Vector<Vector<Box>>& a_OldMeshes)
{
    ...
    CH_assert(a_topLevel >= 0);
    CH_assert(a_baseLevel < (a_topLevel + 1) && a_baseLevel >= 0);
    CH_assert((int)a_OldMeshes.size() >= a_topLevel + 1);
    CH_assert((int)m_vectDomains.size() >= a_topLevel + 1);
    CH_assert((int)m_nRefVect.size() >= a_topLevel + 1);
    CH_assert((int)a_tags.size() >= a_topLevel + 1);
    ...
    if (level >= a_baseLevel) {
        ...
        if (TopLevel + 1 > a_baseLevel) {
            ...
            if (domaint != testdom)
                MayDay::Error(
                    "MeshRefine:domain and Blocking Factor incompatible");
                // [NOTE: the following validations are expensive so only do
                // them when
                //        debugging.]
            ...
            for (int i = a_baseLevel; i <= TopLevel; i++) {
                if (a_OldMeshes[i].size() > 0) {
                    ...
                    for (unsigned int ibox = 0; ibox < a_OldMeshes[i].size();
                         ++ibox) {
            ...
            for (int i = a_baseLevel; i < TopLevel; i++) {
                if (m_vectDomains[i + 1].domainBox() !=
                    ...
                    MayDay::Error(
                        "MeshRefine:domains and refratios incompatible");
            }
            //
            ...
            for (int l = a_baseLevel; l <= TopLevel; ++l) {
                // first intersect tags with domains (note that in periodic
                // case, this can result in wrapping of tags that are outside
                // the base domain box)
                ...
                // pout() << "nBefore: "  << nBefore
                //        << " nAfter:"   << nAfter
                //        << " nTmp: "    << nTmp
                //        << " diff :"    << nBefore - nAfter
                //        << " newDiff: " << nTmp - nAfter << "\n ";

                // coarsen by blocking factor, rounding upwards the goal of
                // this is to coarsen everything down to a level which is
                // m_blockFactor coarser than the new fine level we're going
                // to generate.  By generating grids at this fake level, we
                // can then refine up to the new level, and the blocking
                // factor will be automatically enforced.
                modifiedTags[l].coarsen(m_level_blockfactors[l]);
                // Same block-factor coarsening for the domains
                Domains[l].coarsen(m_level_blockfactors[l]);
                // Same block-factor coarsening for the buffers
                blocked_BufferSize[l] =
                    (m_bufferSize + m_level_blockfactors[l] - 1) /
            ...
            // Note: if m_PNDMode = 1, this only ensures the tags are in the
            // base PND!  Remaining adherence to each level's PND is performed
            // in the box generation algorithm
            for (int lvl = a_baseLevel; lvl <= TopLevel; lvl++) {
                ...
                // pout()<<"nBefore:"<<nBefore<<" nAfter:"<<nAfter<<"
                // diff:"<<nBefore-nAfter<<"\n";
            }
            ...
            // At each level, starting at the top, generate boxes that cover the
            // tag cells using makeBoxes(), refine these boxes and save them in
            // the output \var{a_newmeshes}.  Take the unrefined boxes, add a
            // buffer zone around each box, coarsen, and union with the tag
            // cells on the next coarser level.  This modifies the tags
            // variable.  To handle \var{BlockFactor}, coarsen everything before
            // making the new meshes and then refine the resulting mesh boxes.
            ...
            for (int lvl = TopLevel; lvl >= a_baseLevel; lvl--) {
                ...
                if (procID() == dest_proc) {
                    for (unsigned int i = 0; i < all_tags.size(); ++i) {
                        ...
                        //**The following works around a bug in IVS that appears
                        //if
                        //**the above line is used.  This bug is observed when
                        //there
                        //**is a coarsening of an IVS containing only
                        //IntVect::Zero
                        //**followed by an IVS |= IVS.
                        for (IVSIterator ivsit(all_tags[i]); ivsit.ok();
                             ++ivsit) {
                ...
                // reduce memory -- shouldn't have other effects. (BVS,NDK
                // 6/30/2008) Union the meshes from the previous level with the
                // tags on this level to guarantee that proper nesting is
                // satisfied.  On the first iteration this is a no-op because
                // \var{lvlboxes} is empty. [NOTE: for every iteration after the
                // first, \var{lvlboxes} will
                //        already be coarsened by \var{BlockFactor} so it will
                //        be at the same refinement level as \var{tags[lvl]},
                //        which has also been coarsened]
                ...
                if (lvldomain.isPeriodic()) {
                    ...
                    for (unsigned int i = 0; i < lvlboxes.size(); i++) {
                        ...
                        // enforce proper nesting only do this if original box
                        // was not contained by domainBox
                ...
                const int maxBoxSizeLevel =
                    m_maxSize / (m_level_blockfactors[lvl] * m_nRefVect[lvl]);
                makeBoxes(lvlboxes,
                          modifiedTags[lvl],
                          m_pnds[lvl],
                          lvldomain,
                ...
                // Previously, there were a_tags.makeEmpty() calls in
                // BRMesh.cpp, and now, if there are a few tags leftover here,
                // they will get added onto the mix -- which is not the behavior
                // of the code before these changes (ndk) (leaving commented for
                // now)
                modifiedTags[lvl]
                    .makeEmpty();  // <-- why is this necessary now?

                // refine the new mesh and save it
                //[NOTE: we have to undo the coarsening by \var{BlockFactor} as
                //well
                ...
                for (unsigned int ibox = 0; ibox < a_newmeshes[lvl + 1].size();
                     ++ibox) {
                    a_newmeshes[lvl + 1][ibox].refine(
                        m_level_blockfactors[lvl] * m_nRefVect[lvl]);
                }
                ...
                if (lvl > a_baseLevel) {
                    ...
                    // at that level, then add the buffer cells, clip at the
                    // domain boundaries so we can union them in the next
                    // iteration allInOne_nRef does:
                    ...
                    for (unsigned int ibox = 0; ibox < lvlboxes.size();
                         ++ibox) {
                        lvlboxes[ibox].grow(blocked_BufferSize[lvl]);
                        lvlboxes[ibox] &= Domains[lvl];
                     ...
                     const Vector<int>&           a_bufferSize) const
{
  ...
  CH_assert(   (int)a_domains.size() >= a_topLevel + 1 );
  CH_assert( (int)m_nRefVect.size() >= a_topLevel + 1 );
