...
bool FArrayBox::Seeded=false;
unsigned int FArrayBox::detSeed=0;
                     ...
                     int        a_numcomp) const
{
  ...
  int tmplen = 0;
  Real nrm = 0;
  const Real numPts = static_cast<Real>(a_subbox.numPts());
  ...
  if (a_p == 0)
    ...
    {
        nrm = sqrt(sumPow(a_subbox, 2, a_comp, a_numcomp) / numPts);
    } else {
        // so standard norms weren't good enough for you?
        Real invpwr = 1.0 / a_p;
        nrm = pow(sumPow(a_subbox, a_p, a_comp, a_numcomp) / numPts, invpwr);
    }
...
//-----------------------------------------------------------------------
void
FArrayBox::fillDetRnd(const Real a, const Real b)
{
    if (!(FArrayBox::Seeded))
    {
      FArrayBox::detSeed=procID();
      FArrayBox::Seeded=true;
    }
    std::mt19937                         gen(this->detSeed);
    std::uniform_real_distribution<Real> dist(a, b);

    BoxIterator bit(this->box());
    for (int ncomp = 0; ncomp < this->nComp(); ++ncomp) {
        for (bit.reset(); bit.ok(); ++bit) {
            this->set(bit(), ncomp, dist(gen));
        }
    }
    FArrayBox::detSeed += gen(); // so that next time it is used, it won't return the
    // same sequence.
}
//----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void
FArrayBox::fillRndRnd(const Real a, const Real b)
{
...
#ifdef CH_USE_PYTHON
PyObject *FArrayBox::pack() const {
   unsigned long long size = this->size(this->box(), this->interval());

  PyObject *pView =
      PyMemoryView_FromMemory((char *)this->dataPtr(), size, PyBUF_READ);
    // get the memory address

  PyObject *pArgs = PyTuple_New(5+1);
  ...
  IndexType ind = this->box().ixType();

  for (auto i = 0; i < SpaceDim; ++i) {
    PyTuple_SetItem(pLo, i, PyLong_FromLong(this->box().smallEnd()[i]));
    PyTuple_SetItem(pSize, i, PyLong_FromLong(this->size()[i]));
    PyTuple_SetItem(pCentering, i, PyLong_FromLong(ind.test(i)));
  }
  PyTuple_SetItem(nComp, 0, PyLong_FromLong(this->nComp()));
  ...
  std::string Label = "FAB";
  PyObject *pLabel = PyUnicode_FromString(Label.c_str());
  PyTuple_SetItem(pArgs, 5, pLabel);
  return pArgs;
  // again here we do not decref the items stored in pArgs, since the latter
  // owns them now.
}

PyObject *FArrayBox::pack() {
  unsigned long long size = this->size(this->box(), this->interval());

  PyObject *pView =
      PyMemoryView_FromMemory((char *)this->dataPtr(), size, PyBUF_WRITE);
  // get the memory address
