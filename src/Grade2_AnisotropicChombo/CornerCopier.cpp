...
void
CornerCopier::define(const DisjointBoxLayout& /*a_level*/,
                     const BoxLayout&         /*a_dest*/,
                     bool                     /*a_exchange*/,
                     IntVect                  /*a_shift*/)
{
...
void
CornerCopier::define(const BoxLayout&     a_level,
                     const BoxLayout&     a_dest,
                     const ProblemDomain& a_domain,
                     const IntVect&       a_ghost,
                     bool                 a_exchange,
                     IntVect              a_shift)
{
    if (a_shift != IntVect::Zero) {
        MayDay::Error(
            "CornerCopier::define does not support any a_shift. Do not use.");
    }

    using std::vector;

    const IntVect activeDirs = [&]() {
        IntVect v = IntVect::Unit;
        for (int d = 0; d < SpaceDim; ++d) {
            if (a_domain.size(d) == 1 && a_ghost[d] == 0) {
                v[d] = 0;
            }
        }
        return v;
    ...
    if (!domainBox.isEmpty()) isPeriodic = a_domain.isPeriodic();
      ...
      vdi != vectorDestOnProcDI.end(); ++vdi)
  {
    ...
    {
      if (!activeDirs[dir]) continue; // Added by ES
      ...
      for (faceSit.begin(); faceSit.ok(); ++faceSit)
      {
        ...
        {
          if (!activeDirs[cornerDir]) continue; // Added by ES
  ...
  if (isPeriodic)
  {
    ...
    for (LayoutIterator from(a_level.layoutIterator()); from.ok(); ++from)
    {
      ...
      if (!shrunkDomainBox.contains(fromBox))
      {
        ...
        // in the domain
        for (unsigned int toRef=0; toRef<periodicallyFilledToVect.size(); toRef++)
        {
              ...
              for (int cornerDir = 0; cornerDir<SpaceDim; cornerDir++)
              {
                ...
                if (cornerDir != dir)
                {
                  ...
                  for (cornerSit.begin(); cornerSit.ok(); ++cornerSit)
                  {
                    ...
                    for (unsigned int fromRef = 0; fromRef<periodicFromVect.size(); fromRef++)
                    {
