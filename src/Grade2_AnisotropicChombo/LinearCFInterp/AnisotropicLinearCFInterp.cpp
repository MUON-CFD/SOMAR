#include "AnisotropicLinearCFInterp.H"
#include "AnisotropicFineInterpF_F.H"
...
// -----------------------------------------------------------------------------
// Tests if a_box is near a periodic domain boundary.
// MODIFIED
// -----------------------------------------------------------------------------
                      ...
                      const IntVect&       a_rad)
{
    ...
    for (int idir = 0; idir < SpaceDim; idir++) {
        if (a_pdom.isPeriodic(idir)) {
            if ((Abs(a_box.smallEnd(idir) - domBox.smallEnd(idir)) <= a_rad[idir]) ||
                (Abs(  a_box.bigEnd(idir) - domBox.bigEnd(idir))   <= a_rad[idir])) {
                nearPeriodic = true;
                break;
            }
        }
...
// MODIFIED
// -----------------------------------------------------------------------------
const IntVect AnisotropicLinearCFInterp::s_stencil_radius = IntVect::Unit;
...
// -----------------------------------------------------------------------------
// Full constructor (cannot handle periodic domains)
// MODIFIED
// -----------------------------------------------------------------------------
AnisotropicLinearCFInterp::
AnisotropicLinearCFInterp (const DisjointBoxLayout& a_fine_domain,
                           const DisjointBoxLayout& a_coarse_domain,
                           int a_num_comps,
                           const Box& a_crse_problem_domain,
                           const IntVect& a_ref_ratio,
                           const IntVect& a_interp_radius,
...
// -----------------------------------------------------------------------------
// Full define (cannot handle periodic domains)
// MODIFIED
// -----------------------------------------------------------------------------
void
AnisotropicLinearCFInterp::
...
define (const DisjointBoxLayout& a_fine_domain,
        ...
        bool a_pwconst_interp_only)
{
    CH_TIME("AnisotropicLinearCFInterp::define");
    ...
    const ProblemDomain fine_problem_domain = refine(m_crse_problem_domain, m_ref_ratio);
    ...
    if (a_coarse_domain.isClosed()) {
...

        {
            ...
            const IntVect coarse_slope_radius = (m_interp_radius + m_ref_ratio - IntVect::Unit) / m_ref_ratio;
            const IntVect coarse_ghost_radius = coarse_slope_radius + s_stencil_radius;
...

        {
            ...
            for (dit.begin(); dit.ok(); ++dit) {
                ...
                fine_interp.define(coarsened_fine_interp);
                refine(fine_interp, m_ref_ratio);
...
fillInterp (LevelData<FArrayBox>& a_fine_data,
            ...
            int a_num_comp)
{
    CH_TIME("AnisotropicLinearCFInterp::fillInterp");
    ...
    CH_assert (fineGrids.checkPeriodic(refine(m_crse_problem_domain, m_ref_ratio)));
#endif
...
timeInterp (const LevelData<FArrayBox>& a_old_coarse_data,
            ...
            int a_num_comp)
{
    CH_TIME("AnisotropicLinearCFInterp::timeInterp");
    Interval src_interval (a_src_comp,  a_src_comp  + a_num_comp - 1);
    Interval dest_interval(a_dest_comp, a_dest_comp + a_num_comp - 1);

    if ((a_old_coarse_data.boxLayout().size() == 0) && (a_new_coarse_data.boxLayout().size() == 0)) {
        MayDay::Error ( "AnisotropicLinearCFInterp::fillInterp: no old coarse data and no new coarse data" );
    } else if ((a_time_interp_coef == 1.) || (a_old_coarse_data.boxLayout().size() == 0)) {
        // old coarse data is absent, or fine time level is the new coarse time level
        a_new_coarse_data.copyTo(src_interval,
                                 m_coarsened_fine_data,
                                 dest_interval,
                                 m_coarsenCopier);
    } else if ((a_time_interp_coef == 0.) || (a_new_coarse_data.boxLayout().size() == 0)) {
        ...
        a_new_coarse_data.copyTo(src_interval,
                                 ...
                                 dest_interval,
                                 m_coarsenCopier);
        const DisjointBoxLayout& coarsened_fine_layout = m_coarsened_fine_data.disjointBoxLayout();
        LevelData<FArrayBox> tmp_coarsened_fine_data(coarsened_fine_layout,
...
{
    CH_TIME("AnisotropicLinearCFInterp::fillConstantInterp");
...
computeSlopes (int a_src_comp,
               int a_num_comp)
{
    CH_TIME("AnisotropicLinearCFInterp::computeSlopes");
...
void
AnisotropicLinearCFInterp::
computeSimpleSlopesFab (FArrayBox&        a_slopeFab,
...
{
    CH_TIME("AnisotropicLinearCFInterp::computeSimpleSlopes");
...
void
AnisotropicLinearCFInterp::
computeMultiDimSlopes(FArrayBox&       a_slopes0,
                      FArrayBox&       a_slopes1,
                      FArrayBox&       a_slopes2,
                      const FArrayBox& a_dataFab,
                      const int&       a_src_comp [[maybe_unused]],
                      const int&       a_num_comp [[maybe_unused]],
                      const Box&       a_slopeBox)
{
    // I noticed we take in these parameters, but don't use them!
    // This is not a problem if we always want to operate on all comps.
    // Let's check. If these asserts ever trip, we'll fix accordingly.
    CH_assert(a_src_comp == 0);
    CH_assert(a_num_comp == a_dataFab.nComp());
...
incrementLinearInterp (LevelData<FArrayBox>& a_fine_data,
                       ...
                       int a_num_comp) const
{
    CH_TIME("AnisotropicLinearCFInterp::incrementLinearInterp");
    for (int dir = 0; dir < SpaceDim; dir++) {
        DataIterator dit = a_fine_data.boxLayout().dataIterator();
        for (dit.begin(); dit.ok(); ++dit) {
            ...
            for (ivsit.begin(); ivsit.ok(); ++ivsit) {
                ...
                const int offset = fine_iv[dir] - m_ref_ratio[dir] * coarse_iv[dir];
                Real interp_coef = -.5 + (offset + .5) / m_ref_ratio[dir];

                int coarse_comp = a_src_comp;
                int fine_comp   = a_dest_comp;

                for (; coarse_comp < a_src_comp + a_num_comp; ++coarse_comp, ++fine_comp) {
                    fine_data_fab(fine_iv, fine_comp) += interp_coef * slope_fab(coarse_iv, coarse_comp);
                }
            }
        }
    }
