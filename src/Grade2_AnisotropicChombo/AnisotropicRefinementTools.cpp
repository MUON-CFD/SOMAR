...
coarsenToIsotropy(ProblemDomain&       a_isoDomain,
                  ...
                  const RealVect&      a_L)
{
    const RealVect dXi = a_L / RealVect(a_domain.size());

    // How much do we need to coarsen in each direction to reach isotropy?
    D_TERM(
    int maxDir = 0;,
    if (dXi[1] > dXi[maxDir]) maxDir = 1;,
    if (dXi[2] > dXi[maxDir]) maxDir = 2;)

    a_refToIsotropy = IntVect::Unit;
    D_TERM(
    a_refToIsotropy[0] = int(dXi[maxDir] / dXi[0]);,
    a_refToIsotropy[1] = int(dXi[maxDir] / dXi[1]);,
    a_refToIsotropy[2] = int(dXi[maxDir] / dXi[2]);)

    // Once we coarsen the domain by refToIsotropy, we will end up with
    // the isoDomain.
    coarsen(a_isoDomain, a_domain, a_refToIsotropy);

    TODONOTE("Print this data somewhere.");
    // pout() << "domain.size() = " << a_domain.size() << endl;
    // pout() << "dXi = " << dXi << endl;
    // pout() << "refToIsotropy = " << a_refToIsotropy << endl;
    // pout() << "isotropic dXi = " << dXi * a_refToIsotropy << endl;
    // pout() << "isoDomain.size() = " << a_isoDomain.size() << endl;

    // BasicIO::tout(0) << "domain.size() = " << a_domain.size() << endl;
    // BasicIO::tout(0) << "dXi = " << dXi << endl;
    // BasicIO::tout(0) << "refToIsotropy = " << a_refToIsotropy << endl;
    // BasicIO::tout(0) << "isotropic dXi = " << dXi * a_refToIsotropy << endl;
    // BasicIO::tout(0) << "isoDomain.size() = " << a_isoDomain.size() << endl;
}
                  ...
                  const IntVect&   a_refRatio)
{
    ...
    for (unsigned int idx = 0; idx < boxes.size(); ++idx) {
        Box b = boxes[idx];
        b.coarsen(a_refRatio);
        b.refine(a_refRatio);
              ...
              const IntVect&           a_refinement)
{
    ...
    if (a_refinement == IntVect::Unit) {
        a_output = a_input;
        return;
    }

    if (!coarsenable(a_input, a_refinement)) {
        MAYDAYERROR("Grids cannot be coarsened by ref = " << a_refinement);
    }
    if (!a_input.isClosed()) {
        MAYDAYERROR("input to coarsen must be called with closed BoxLayout");
    }
    if (a_output.isClosed()) {
        MAYDAYERROR("output of coarsen must be called on open BoxLayout");
    }
    ...
    unsigned int j = 0;
    if (boxes.size() > (unsigned int)3) {
        for (unsigned int i = 0; i <= boxes.size() - 4; i += 4) {
