    ...
    Side::LoHiSide a_hiorlo)
{
    ...
    for (size_t i = 0; i < a_periodicVector.size(); ++i) {
                                    ...
                                    const DisjointBoxLayout& a_fineBoxes)
{
    ...
    std::sort(a_periodicVector.begin(), a_periodicVector.end());
}
...
*/
Real
MappedQuadCFStencil::computeMixedDerivative(const BaseFab<Real>& a_phic,
                                            int                  a_ivar,
                                            const IntVect&       a_ivin,
                                            const RealVect&      a_dx) const
{
    ...
    CH_assert(a_ivar < a_phic.nComp());
    CH_assert(!a_phic.box().isEmpty());
    D_TERM(CH_assert(a_dx[0] > 0);, CH_assert(a_dx[1] > 0);
           , CH_assert(a_dx[2] > 0);)
    ...
    if (m_ivsStandard.contains(a_ivin)) {
        ...
        mixedd     = (phiur + phill - philr - phiul) /
                 (4. * a_dx[itran1] * a_dx[itran2]);
    } else {
        CH_assert(m_ivsQuadd.contains(a_ivin));
        CH_assert(m_dropOrd.box().contains(a_ivin));
        if (m_dropOrd(a_ivin)) {
...
void
MappedQuadCFStencil::define(const ProblemDomain&     a_fine_domain,
                            const Box&               a_grid,
                            const DisjointBoxLayout& a_fineBoxes,
                            const DisjointBoxLayout& a_coarBoxes,
                            const IntVect&           a_refRatio,
                            ...
                            bool                     a_isFlat)
{
    ...
    D_TERM(CH_assert(a_refRatio[0] >= 1);, CH_assert(a_refRatio[1] >= 1);
           , CH_assert(a_refRatio[2] >= 1);)
    CH_assert(a_direction >= 0);
    CH_assert(a_direction < SpaceDim);
