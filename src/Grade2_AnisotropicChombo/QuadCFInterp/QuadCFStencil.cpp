...
*/
Real
QuadCFStencil::computeSecondDerivative(const BaseFab<Real>& a_phic,
                                       int                  a_derivdir,
                                       int                  a_ivar,
                                       ...
                                       Real                 a_dx) const
{
    ...
    if (m_ivsStandard.contains(a_ivin)) {
        ...
        if (m_dropOrd(a_ivin)) {
            ...
            const BaseFab<DerivStencil>& stenfab = m_second[a_derivdir];
...
Real
QuadCFStencil::computeFirstDerivative(const BaseFab<Real>& a_phic,
                                      int                  a_derivdir,
                                      int                  a_ivar,
                                      const IntVect&       a_ivin,
                                      Real                 a_dx) const
{
    ...
    if (m_ivsStandard.contains(a_ivin)) {
        ...
        const BaseFab<DerivStencil>& stenfab = m_firstD[a_derivdir];
        CH_assert(stenfab.box().contains(a_ivin));
        const DerivStencil& dersten = stenfab(a_ivin);
        firstd                      = 0.0;
        bool keepzer                = false;
...
*/
Real
QuadCFStencil::computeMixedDerivative(const BaseFab<Real>& a_phic,
                                      int                  a_ivar,
                                      const IntVect&       a_ivin,
                                      Real                 a_dx) const
{
...
void
QuadCFStencil::define(const Box&               a_fine_domain,
                      const Box&               a_grid,
                      const DisjointBoxLayout& a_fineBoxes,
                      const DisjointBoxLayout& a_coarBoxes,
                      int                      a_refRatio,
...
void
QuadCFStencil::define(const ProblemDomain&     a_fine_domain,
                      const Box&               a_grid,
                      const DisjointBoxLayout& a_fineBoxes,
                      const DisjointBoxLayout& a_coarBoxes,
                      int                      a_refRatio,
                      ...
                      Side::LoHiSide           a_hiorlo)
{
    ...
    if (coarseDomain.isPeriodic())
        for (int idir = 0; idir < SpaceDim; idir++)
            if (coarseDomain.isPeriodic(idir)) periodicTestBox.grow(idir, -1);
                      ...
                      Side::LoHiSide           a_hiorlo)
{
    ...
    for (size_t i = 0; i < a_coarsenedPeriodicBoxes.size(); ++i) {
...
void
QuadCFStencil::buildStencils(const IntVectSet& ivsAllGood)
{
#if CH_SPACEDIM == 1
    // this shouldn't ever be used, but it's simpler
    // to keep it around rather than block out every
    ...
    for (IVSIterator irregit(m_ivsQuadd); irregit.ok(); ++irregit) {
...
*/
void
QuadCFStencil::addFabToSten(const BaseFab<Real>& a_fabin, DerivStencil& a_sten)
{
    CH_assert(isDefined());
