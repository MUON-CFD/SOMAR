...
///get iv at ivec
const IntVect&
DerivStencil::getIndex(size_t a_ivec) const
{
    CH_assert(isdefined);
...
///get weight at ivec
const Real&
DerivStencil::getWeight(size_t a_ivec) const
{
    CH_assert(isdefined);
    CH_assert(a_ivec < m_vectIV.size());
    ...
    )
{
    ...
    size_t ithere= size_t(-1);
    for (size_t ivect = 0; ivect < m_vectIV.size(); ivect++)
    {
...
DerivStencil::operator+=(Real a_facin)
{
    CH_assert(isdefined);
    CH_assert(m_vectIV.size()==m_vectWgt.size());
    for (size_t ivec = 0; ivec < m_vectWgt.size(); ivec++)
    {
