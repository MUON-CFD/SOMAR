...
#include "Integral.H"  // BUG: This is defined in Grade 3
#include "Debug.H"
...
BoundaryData<Real>::BoundaryData (const DisjointBoxLayout& a_grids,
                                  ...
                                  const int                a_ncomp,
                                  const SideArray          a_doSides)
{
    this->define(a_grids, a_domain, a_ncomp, a_doSides);
}
                                 ...
                                 const SideArray          a_doSides)
{
    ...
    for (int dir = 0; dir < SpaceDim; ++dir) {
        ...
        for (sit.reset(); sit.ok(); ++sit) {
            ...
            // Does the user want to work with this boundary?
            if (!a_doSides[dir][s]) continue;
...
{
    this->clear();
    if (!a_src.hasGrids()) return;

    m_grids = a_src.m_grids;
    m_ncomp = a_src.m_ncomp;
...
// -----------------------------------------------------------------------------
void BoundaryData<Real>::regrid (const DisjointBoxLayout& a_newGrids)
{
    // Copy old data to temp holder.
    LevelData<FluxBox> oldData(m_grids, m_ncomp);
    setValLevel(oldData, quietNAN);
    this->copyTo(oldData);

    // Send old data to new grids.
    LevelData<FluxBox> newData(a_newGrids, m_ncomp);
    setValLevel(newData, quietNAN);
    oldData.copyTo(newData);

    // Redefine this object and copy data.
    CH_assert(a_newGrids.physDomain() == m_grids.physDomain());
    this->define(a_newGrids, a_newGrids.physDomain(), m_ncomp);
    this->copy(newData);
}
...
// Returns the layout.
// -----------------------------------------------------------------------------
const DisjointBoxLayout& BoundaryData<Real>::getBoxes() const
{
    CH_assert(this->hasGrids());
    return m_grids;
}


// -----------------------------------------------------------------------------
// Coarsens this BoundaryData object and it's data.
// -----------------------------------------------------------------------------
void BoundaryData<Real>::coarsen (const IntVect&                  a_refRatio,
                                  const LevelData<FluxBox>* const a_fineJPtr)
{
    // Copy old data to temp holder.
    LevelData<FluxBox> fineData(m_grids, m_ncomp);
    setValLevel(fineData, quietNAN);
    this->copyTo(fineData);
    ...
    // Send data to coarser LevelData.
    LevelData<FluxBox> crseData(crseGrids, m_ncomp);
    CFInterp::localCoarsen(crseData, fineData, a_fineJPtr);

    // Redefine this object and copy data.
    this->define(crseGrids, crseGrids.physDomain(), m_ncomp);
    this->copy(crseData);
}
...
// -----------------------------------------------------------------------------
void BoundaryData<Real>::coarsen (const DisjointBoxLayout&        a_crseGrids,
                                  const LevelData<FluxBox>* const a_fineJPtr)
{
    // Copy old data to temp holder.
    LevelData<FluxBox> fineData(m_grids, m_ncomp);
    ...
    // Send data to coarser LevelData.
    CFInterp interpObj;
    const RealVect dXiDummy(D_DECL(1.0, 1.0, 1.0));
    interpObj.define(m_grids, dXiDummy, a_crseGrids);

    LevelData<FluxBox> crseData(a_crseGrids, m_ncomp);
    interpObj.localCoarsen(crseData, fineData, a_fineJPtr);

    // Redefine this object and copy data.
    this->define(a_crseGrids, a_crseGrids.physDomain(), m_ncomp);
    this->copy(crseData);
    ...
    for (int dir = 0; dir < SpaceDim-1; ++dir) {
        ...
        for (sit.reset(); sit.ok(); ++sit) {
            ...
            while (thisIt != endIt) {
                ...
                const Box& thisFlatBox =
                    Subspace::flattenBox(thisIt->second->box(), SpaceDim - 1);
...
// -----------------------------------------------------------------------------
bool BoundaryData<Real>::hasGrids () const
{
    return m_grids.isClosed();
}
...
// -----------------------------------------------------------------------------
int BoundaryData<Real>::nComp () const
{
    return m_ncomp;
}
                                      ...
                                      const Real                  a_loMult) const
{
    ...
    Real intRhs = Integral::sum(a_Jrhs, localDx, 0); // BUG:: This is defined in Grade 3!
...
// -----------------------------------------------------------------------------
void BoundaryData<Real>::copyAverage (const LevelData<FArrayBox>& a_src)
{
    // Sanity checks
    CH_assert(a_src.isDefined());
    CH_assert(a_src.getBoxes().compatible(m_grids));
    ...
    for (dit.reset(); dit.ok(); ++dit) {
        for (int dir = 0; dir < SpaceDim; ++dir) {
            ...
            for (sit.reset(); sit.ok(); ++sit) {
                ...
                const int numComps = destFABPtr->nComp();
                CH_assert(srcFAB.nComp() == numComps);

                destFABPtr->setVal(0.0);
                destFABPtr->shiftHalf(dir, 1);
                destFABPtr->plus(srcFAB, 0.5, 0, 0, numComps);
                destFABPtr->shift(dir, -1);
                destFABPtr->plus(srcFAB, 0.5, 0, 0, numComps);
                destFABPtr->shiftHalf(dir, 1);
                                                        ...
                                                        const Side::LoHiSide            a_side)
{
    ...
    for (dit.reset(); dit.ok(); ++dit) {
        ...
        LayoutTools::VertShifter<Real> shifty(bdryFAB, srcFAB);
    ...
    for (int dir = 0; dir < SpaceDim-1; ++dir) {
        ...
        for (sit.reset(); sit.ok(); ++sit) {
            ...
            for (; oldIt != oldMap.end(); ++oldIt) {
                ...
                Box newBox = Subspace::flattenBox(oldBox, SpaceDim-1);
