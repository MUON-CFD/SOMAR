...
// -----------------------------------------------------------------------------
AnisotropicMeshRefine::AnisotropicMeshRefine ()
{
    // Do nothing special.
...
// -----------------------------------------------------------------------------
AnisotropicMeshRefine::AnisotropicMeshRefine (const Box&             a_baseDomain,    // Level 0 domain
...
// Full constructor -- leaves object in usable state
// -----------------------------------------------------------------------------
AnisotropicMeshRefine::AnisotropicMeshRefine (const ProblemDomain&   a_baseDomain,    // Level 0 domain
                                              const Vector<IntVect>& a_refRatios,     // Refinement ratios -- refRatio[0] is btwn levels 0 and 1
                                              const Real             a_fillRatio,     // Measure of how efficiently tagged cells will be covered
                                              const int              a_blockFactor,   // Amount by which grids are guaranteed to be coarsenable
                                              const int              a_bufferSize,    // Proper nesting buffer amount
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::define (const Box&             a_baseDomain,     // Level 0 domain
                                    const Vector<IntVect>& a_refRatios,      // Refinement ratios -- refRatio[0] is btwn levels 0 and 1
                                    const Real             a_fillRatio,      // Measure of how efficiently tagged cells will be covered
                                    const int              a_blockFactor,    // Amount by which grids are guaranteed to be coarsenable
                                    const int              a_bufferSize,     // Proper nesting buffer amount
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::define (const ProblemDomain&   a_baseDomain,     // Level 0 domain
                                    const Vector<IntVect>& a_refRatios,      // Refinement ratios -- refRatio[0] is btwn levels 0 and 1
                                    const Real             a_fillRatio,      // Measure of how efficiently tagged cells will be covered
                                    const int              a_blockFactor,    // Amount by which grids are guaranteed to be coarsenable
                                    const int              a_bufferSize,     // Proper nesting buffer amount
...
// -----------------------------------------------------------------------------
int
AnisotropicMeshRefine::regrid(
    Vector<Vector<Box> >&       /*a_newmeshes*/,  // new set of grids at every level
...
{
    MayDay::Error("AnisotropicMeshRefine::regrid 1 not yet written");
...
AnisotropicMeshRefine::regrid(
    ...
    const Vector<Vector<Box> >& a_OldMeshes)  // existing grids (if no previous grids, set to domains)
{
    CH_TIME("AnisotropicMeshRefine::regrid");
    //
    ...
    CH_assert( static_cast<int>(a_OldMeshes.size()) >= a_topLevel + 1 );
    CH_assert( static_cast<int>(m_vectDomains.size()) >= a_topLevel + 1 );
    CH_assert( static_cast<int>(m_nRefVect.size()) >= a_topLevel + 1 );       // TODO: Relax this restriction.
    CH_assert( static_cast<int>(a_tags.size()) >= a_topLevel+1 );
    ...
    if (m_spanDirs.sum() > 0) {
        ...
        for (int lev = a_baseLevel; lev <= a_topLevel; ++lev) {
            ...
            bool recheck = AnisotropicMeshRefine::spanBoxes(stretchedOldMeshes[lev],
    ...
    int new_finest_level = -1;  // This is the return value.
    int isize            = a_tags.size();

    const int level = [&]() {
        int l = Min(a_topLevel, isize - 1);
        for (; l >= a_baseLevel; --l) {
            if (!a_tags[l].isEmpty()) break;
        }

#ifdef CH_MPI
        {
            int rcvl;
            MPI_Allreduce(&l, &rcvl, 1, MPI_INT, MPI_MAX, Chombo_MPI::comm);
            l = rcvl;
        }
#endif
        return l;
    ...
    if (level >= a_baseLevel) {
        ...
        if ( TopLevel+1 > a_baseLevel ) {
            ...
            if (domaint != testdom)
                MayDay::Error("AnisotropicMeshRefine:domain and Blocking Factor incompatible");
            ...
            for ( int i = a_baseLevel ; i <= TopLevel ; i++ ) {
                if (oldMeshesRef[i].size() > 0) {
                    ...
                    for (unsigned int ibox = 0; ibox < oldMeshesRef[i].size(); ++ibox) {
            ...
            for ( int i = a_baseLevel ; i < TopLevel ; i++ ) {
                if (m_vectDomains[i+1].domainBox() != refine(m_vectDomains[i].domainBox(), m_nRefVect[i]))
                    MayDay::Error("AnisotropicMeshRefine:domains and refratios incompatible");
            }
            ...
            for (int l = a_baseLevel; l <= TopLevel; ++l) {
                ...
                // int nBefore = modifiedTags[l].numPts();
                // IntVectSet tmp(modifiedTags[l]);
                //tmp &= m_vectDomains[l].domainBox();
                //int nTmp = tmp.numPts();
                modifiedTags[l] &= m_vectDomains[l];
                //int nAfter = modifiedTags[l].numPts();
                ...
                // coarsen everything down to a l which is m_blockFactor coarser than
                // the new fine l we're going to generate.  By generating grids at
                // this fake l, we can then refine up to the new l, and the blocking
                // factor will be automatically enforced.

                // TODO: This is a bottleneck!
                // CH_assert(   m_level_blockfactors[l][0] == m_level_blockfactors[level][1]
                //           && m_level_blockfactors[l][0] == m_level_blockfactors[level][2]   );
                {
                    CH_TIME("Coarsen_modifiedTags");
                    coarsen(modifiedTags[l], m_level_blockfactors[l]);
                    // modifiedTags[l].coarsen(m_level_blockfactors[l][0]);
                ...
                Box newDomBox(Domains[l].domainBox());
                newDomBox.coarsen(m_level_blockfactors[l]);
                bool isPeriodic[CH_SPACEDIM];
                D_TERM(isPeriodic[0] = Domains[l].isPeriodic(0);,
                       isPeriodic[1] = Domains[l].isPeriodic(1);,
                       isPeriodic[2] = Domains[l].isPeriodic(2);)
                Domains[l].define(newDomBox, isPeriodic);

                // Same block-factor coarsening for the buffers
                blocked_BufferSize[l] =
                    (  m_level_blockfactors[l]
            ...
            Vector<Box> OldBaseMesh = oldMeshesRef[a_baseLevel];
            {
                ...
                for (unsigned int i = 0; i!= OldBaseMesh.size(); ++i) {
            ...
            for ( int lvl = TopLevel ; lvl >= a_baseLevel ; lvl-- ) {
                ...
                if (lvldomain.isPeriodic() ) {
                    ...
                    for (unsigned int i=0; i<lvlboxes.size(); i++) {
                ...
                for (unsigned int ibox = 0; ibox < a_newmeshes[lvl+1].size(); ++ibox) {
                ...
                if ( lvl > a_baseLevel ) {
                    ...
                    for (unsigned int ibox = 0 ; ibox < lvlboxes.size() ; ++ibox) {
    ...
    if (m_spanDirs.sum() > 0) {
        ...
        //     bool recheck = AnisotropicMeshRefine::spanBoxes(a_newmeshes[lev],
        //                                                m_vectDomains[lev].domainBox(),
        //                                                m_spanDirs);
        //     if (recheck) --lev;
        // }
...
// avoid an overlap.
// -----------------------------------------------------------------------------
bool AnisotropicMeshRefine::spanBoxes (Vector<Box>&   a_boxes,
                                       const Box&     a_domBox,
                                       const IntVect& a_spanDirs)
{
    CH_TIME("AnisotropicMeshRefine::spanBoxes");
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::domainSplit (const ProblemDomain& a_domain,
                                         Vector<Box>&         a_vbox,
                                         const IntVect&       a_maxBoxSize,
                                         int                  a_blockFactor)
{
    const Box& domBox = a_domain.domainBox();
    AnisotropicMeshRefine::domainSplit(domBox, a_vbox, a_maxBoxSize, a_blockFactor);
}
...
// If a_maxBoxSize[i] == 0, the domain will not be split in the ith direction.
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::domainSplit (const Box&     a_domain,
                                         Vector<Box>&   a_vbox,
                                         const IntVect& a_maxBoxSize,
                                         int            a_blockFactor)
{
    ...
    if (refine(d, maskedBF) != a_domain) {
        MayDay::Error("AnisotropicMeshRefine::domainSplit: a_domain not coarsenable by blockingFactor");
    }
    ...
    for (int i = 0; i < CH_SPACEDIM; ++i) {
        ...
        AnisotropicMeshRefine::breakBoxes(a_vbox, coarsenedMaxBoxSize, i);
    }

    // Finally, refine our results back to the original domain size.
    for (unsigned int i = 0; i< a_vbox.size(); ++i) {
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::makeBoxes (Vector<Box>&         a_mesh,
...
{
    CH_TIME("AnisotropicMeshRefine::makeBoxes");
    std::list<Box> boxes;
    ...
    for (unsigned int i = 0; i< a_mesh.size(); ++i, ++it) a_mesh[i] = *it;
}
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::makeBoxes (std::list<Box>&      a_mesh,
                                       IntVectSet&          a_tags,
                                       const IntVectSet&    a_pnd,
                                       const ProblemDomain& a_domain,
                                       const IntVect&       a_maxBoxSize,
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::makeBoxesParallel (std::list<Box>&      a_mesh,
                                               IntVectSet&          a_tags,
                                               const IntVectSet&    a_pnd,
                                               const ProblemDomain& a_domain,
                                               const IntVect&       a_maxBoxSize,
...
// -----------------------------------------------------------------------------
bool AnisotropicMeshRefine::properlyNested (const Box&           a_box,
                                            const ProblemDomain& a_domain,
                                            const IntVectSet&    a_pnd,
                                            const IntVect&       a_totalBufferSize) const
{
    if (m_PNDMode == 0) {
        MayDay::Error("AnisotropicMeshRefine::properlyNested: m_PNDMode should not be zero");
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::splitTagsInBestDimension(IntVectSet&    a_tags_inout_lo,
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::splitBox (std::list<Box>&                 a_boxes,
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::computeLocalBlockFactors ()
{
    for (unsigned int lev = 0; lev < (m_level_blockfactors.size()); ++lev) {
        ...
        for (int dir = 0; dir < CH_SPACEDIM; ++dir) {
            if (!AnisotropicMeshRefine::isPower2(m_level_blockfactors[lev][dir])) {
                ...
                MayDay::Error("aborting AnisotropicMeshRefine::regrid");
            }
        }
    }
}
...
///////////////////////////////////////////////////////////////////////////////
void
AnisotropicMeshRefine::makePNDs(
    Vector<IntVectSet>&          a_pnds,
    Vector<IntVect>&             a_totalBufferSize,
    const int                    a_baseLevel,
    const int                    a_topLevel,
    const Vector<ProblemDomain>& a_domains[[maybe_unused]],
    const IntVectSet&            a_baseMesh,
    const Vector<IntVect>&       a_bufferSize) const
{
    // Validate inputs
    CH_assert( a_baseLevel <= a_topLevel && a_baseLevel >= 0 );
    CH_assert( static_cast<int>(a_domains.size()) >= a_topLevel + 1 );
    CH_assert( static_cast<int>(m_nRefVect.size()) >= a_topLevel ); // ES: Changed from toplevel+1
    ...
    for ( int lvl = a_baseLevel ; lvl <= a_topLevel ; lvl++) {
        ...
        if (m_PNDMode == 0) {
            MayDay::Error("AnisotropicMeshRefine::m_PNDMode should not be zero");
...
void AnisotropicMeshRefine::makePNDs (Vector<IntVectSet>&          a_pnds,
                                      Vector<IntVect>&             a_totalBufferSize,
                                      const int                    a_baseLevel,
                                      const int                    a_topLevel,
...
{
    IntVectSet mesh;
    for (unsigned int box = 0; box < a_oldMeshes.size(); ++box) {
...
// -----------------------------------------------------------------------------
void AnisotropicMeshRefine::breakBoxes (Vector<Box>& a_vboxin,
...
int* AnisotropicMeshRefine::s_recBuffer = NULL;

void AnisotropicMeshRefine::deleteBuffer ()
{
#ifdef CH_MPI
    if (s_recBuffer != NULL) {
        freeMT(s_recBuffer);
...
void AnisotropicMeshRefine::receiveBoxesParallel (const Interval& a_from,
                                                  const Interval& a_to,
                                                  std::list<Box>& a_mesh,
                                                  int             tag) const
{
