...
// -----------------------------------------------------------------------------
Box LepticBoxTools::FlattenTransform::operator() (const Box& a_inputBox)
{
    constexpr int vdir = SpaceDim - 1;
...
// -----------------------------------------------------------------------------
IntVect LepticBoxTools::getMaxBoxSize (const Vector<Box>& a_boxes)
{
                                            ...
                                            const IntVect&     a_blockFactor)
{
    ...
    for (size_t idx = 0; idx < a_vertBoxes.size(); ++idx) {
        const Box& b = a_vertBoxes[idx];

        if (LepticBoxTools::vertSpanCheck(b, a_vertDomBox)) {
            CH_assert(coarsenable(b, a_blockFactor));
            a_horizBoxes.push_back(Subspace::flattenBox(b, SpaceDim-1));
        }
    }
}
...
// -----------------------------------------------------------------------------
void LepticBoxTools::createLevelGrids (Vector<Box>&       a_flatBoxes,         // Out: For the horizontal solver
...
{
    CH_TIME("LepticBoxTools::flattenLevelGrids");
    using std::list;
    using std::copy;

    // If there are no boxes, we have nothing to do.
    size_t numBoxes = a_origBoxes.size();
    ...
    for (size_t idx = 0; idx < a_preInflatedBoxes.size(); ++idx) {
    ...
    for (size_t idx = 0; dbit != disjointBoxList.end(); ++dbit, ++idx) {
...
// -----------------------------------------------------------------------------
void LepticBoxTools::joinLevelGrids (std::list<Box>&    a_joinList,
                                     const IntVect&     a_joinDirs,
                                     const Vector<Box>& a_fndVector)
{
    CH_TIME("LepticBoxTools::joinLevelGrids");
    ...
    if (a_fndVector.size() > 0) {
        ...
        list<Box> fndList;
        copy(a_fndVector.begin(), a_fndVector.end(), back_inserter(fndList));
...
// -----------------------------------------------------------------------------
void LepticBoxTools::inflateLevelGrids (std::list<Box>&       a_inflatedList,
...
// -----------------------------------------------------------------------------
void
LepticBoxTools::copyProcArray(
    Vector<int>&                          a_destProcArray,
    ...
    const std::list<std::pair<int, Box>>& a_srcAssignments)
{
    ...
    for (int idx = 0; idx < numDestBoxes; ++idx) {
        if (a_destProcArray[idx] < 0) {
            pout() << "LepticBoxTools::copyProcArray: a_destProcArray = "
                   << a_destProcArray << endl;
            MayDay::Error(
                "a_destProcArray not properly assigned in "
                "LepticBoxTools::copyProcArray. See pout");
        }
    }
#endif
}
...
// -----------------------------------------------------------------------------
void LepticBoxTools::createDisjointBoxList (std::list<Box>&    a_disjointBoxList,
...
// -----------------------------------------------------------------------------
bool LepticBoxTools::resolveQuadrants (std::list<Box>&   a_quadrantList,
                                       const Box&        a_testBox,
                                       const Vector<Box> a_boxArray)
{
...
// -----------------------------------------------------------------------------
bool LepticBoxTools::checkOverlap (const Box&        a_testBox,
                                   const Vector<Box> a_boxArray)
{
...
// -----------------------------------------------------------------------------
Box LepticBoxTools::minBox (const Vector<Box>& a_boxArray,
...
LepticBoxTools::replace (std::list<T>&                    a_list,
...
// -----------------------------------------------------------------------------
Box LepticBoxTools::join (const Box& a_box1,
                          const Box& a_box2)
{
    Box retBox = a_box1;
...
// -----------------------------------------------------------------------------
bool LepticBoxTools::fastJoin (Box&       a_box1,
                               const Box& a_box2)
{
