...
UserMain::oneTimeSetup ()
{
    ...
    // For example,
    //    DEMMap* geoPtr = new DEMMap("myDEMFile.hdf5");
    // would create a terrain-following coordinate system from a Digital
    // Elevation Map.

    CartesianMap* geoPtr = new CartesianMap();
    // StretchedMap* geoPtr = new StretchedMap();
...
UserMain::createPhysics ()
{
    ...
    DoubleDiffusionPhysics* physPtr = new DoubleDiffusionPhysics();
    return physPtr;
}
